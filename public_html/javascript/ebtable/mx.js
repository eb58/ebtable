...
const mx = (m, groupDef) => {  //groupdef see below
    ...
    //####################################  filtering #######################
    const filtering = (() => {
        const fcts = { // filters [{col, searchText, renderer}, ...]
            rowMatch: (filters) => (row) => (Array.isArray(filters) ? filters : [filters]).reduce((acc, f) => {
                const cellData = $.trim(row[f.col]);
    ...
    const res = _.extend(m, basicApi, sorting, filtering, grouping, paging);
